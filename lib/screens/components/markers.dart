...
import 'package:oden_app/screens/map_page.dart';
...
void setController(controllerCompleter) async => controller = await controllerCompleter.future;
...
Future<Marker> Function(Cluster<PublicArt>) get _markerBuilder => (cluster) async {
  ...
  return Marker(
    ...
    infoWindow: InfoWindow(
        onTap: () => navigateToDetailsPage(mapsPage.context),
        title: art.name,
...
Future<BitmapDescriptor> _getClusterBitmap(int size,
  ...
  canvas.drawCircle(Offset(size / offsetSizeDivisor, size / offsetSizeDivisor), size / innerCircleRadiusDivisor, black);
  canvas.drawCircle(Offset(size / offsetSizeDivisor, size / offsetSizeDivisor), size / outerCircleRadiusDivisor, green);
  ...
  if (text != null) {
    ...
    painter.paint(
      canvas,
      Offset(size / offsetSizeDivisor - painter.width / offsetSizeDivisor, size / offsetSizeDivisor - painter.height / offsetSizeDivisor),
    );
  } else {
